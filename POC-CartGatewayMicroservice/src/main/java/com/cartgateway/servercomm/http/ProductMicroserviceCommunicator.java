...
public class ProductMicroserviceCommunicator {
	...
	@Value("${PATH-TO-PRODUCT-MICROSERVICE:http://localhost:8887/api/v1}")
	...
	public void sendRequestToProductMicroservice(
		...
		if(future.statusCode()>226) {
			logger.info(message+HttpStatus.valueOf(future.statusCode())+" ["+HttpStatus.valueOf(future.statusCode()).getReasonPhrase()+"]"+
					" Message from Product Microservice: "+mapper.readValue(future.body(), ErrorResponse.class).getMessage()
					);
			throw new ProcessFailedException(
					HttpStatus.valueOf(future.statusCode()),
					mapper.readValue(future.body(), ErrorResponse.class).getMessage().toString());
		}
